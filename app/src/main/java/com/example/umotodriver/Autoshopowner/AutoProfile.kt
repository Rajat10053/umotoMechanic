...
import com.example.umotodriver.databinding.ActivityAutoProfileBinding
...
import kotlinx.android.synthetic.main.activity_auto_profile.*
import kotlinx.android.synthetic.main.activity_profilepage.*
import kotlinx.android.synthetic.main.activity_profilepage.logoutprofile
import java.util.*

class AutoProfile : AppCompatActivity() {
    lateinit var imageUri: Uri
    private lateinit var storage: StorageReference
    private lateinit var binding: ActivityAutoProfileBinding
    private lateinit var auth: FirebaseAuth
    private lateinit var databaseReference: DatabaseReference
    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        binding = ActivityAutoProfileBinding.inflate(layoutInflater)
        setContentView(binding.root)
        auth = FirebaseAuth.getInstance()
        val uId = auth.currentUser?.uid

        databaseReference = FirebaseDatabase.getInstance().getReference("AutoShopOwner")

        binding.autiSelectImege.setOnClickListener {
            selectImage()
        }
        ...
        autobackButton.setOnClickListener {
        ...
        binding.Save.setOnClickListener {
            ...
            val userName = binding.autoProfileName.text.toString()
            val email = binding.autoProfileemail.text.toString()
            val mobileno = binding.autoProfilemobileno.text.toString()
            val Password = binding.autoProfilpassword.text.toString()
                ...
                databaseReference.child(userName).setValue(user).addOnCompleteListener {
                    if (it.isSuccessful){
                        ...
                        databaseReference = FirebaseDatabase.getInstance().reference
                            .child("AutoShopOwner").child(userName!!)
    ...
    private fun uplodeImage() {
        ...
        val storageReference = FirebaseStorage.getInstance().getReference("AutoShopOwner/"+auth.currentUser?.uid)

        storageReference.putFile(imageUri).addOnSuccessListener {
            binding.autofaceimage.setImageURI(null)
            //hideProgressBar()
    ...
    override fun onActivityResult(requestCode: Int, resultCode: Int, data: Intent?) {
        ...
        if (requestCode == 100 && resultCode == RESULT_OK){
            ...
            imageUri = data?.data!!
            binding.autofaceimage.setImageURI(imageUri)
